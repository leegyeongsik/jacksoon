...
final class RouterExceptionSupport {
    private RouterExceptionSupport() {
    }

    static void respond(IOStore ioStore, RouterExceptionContext context, int statusCode, String reason) {
        if (context == null) {
            return;
        }
        SelectionKey key = context.getClientSelectionKey();
        ...
        ioStore.offer(key, new ResponseContext(current.get(), response, false));
    }

    static void closeClient(RouterExceptionContext context) {
        if (context == null) {
            return;
        }
        closeSelectionKey(context.getClientSelectionKey());
    }
